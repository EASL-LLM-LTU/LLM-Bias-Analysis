...
- For 300 outputs × 4 streams, that’s 600 per stream (~2,400 total). It sounds like a lot, but its 400 HTML pages. Feasible.  
- Variance/uncertainty will still guide resurfacing of items within those 600, but stopping is based on the cap, not variance.  
...
Problems:
- Math tampering: Introduces a new similarity term not in Sakaguchi’s formulation — could distort the efficiency proofs of EASL.  
- Overweighting semantics: Embeddings may group together outputs that already have low variance → wasting labels.  
- Parameter tuning needed: Choice of $\lambda\$ is arbitrary; small changes may affect grouping a lot.  
- Scope creep: Adds complexity (particulary experimentation with different lambda) that may not translate into clear performance gains within our project budget.

</details>

### Recommendation
**Option A (scalar-only).**  
Stay faithful to EASL’s proven math. Use variance + mode-driven matching, grouped by `prompt_id` in Round-1 for clarity, then allow EASL to adaptively surface uncertain items with similar scalar ratings.  

Option B is innovative but risks adding unnecessary complexity, could introduce more scope creep and could undermine the clean efficiency guarantees of the original framework.
...
<details>
<summary><strong>Option 1: Compare the 3 models (substantive)</strong></summary>

Goal: See which LLM/LMM shows more/less bias.

- Compute per-model means/medians of item modes.  
- Report share of outputs rated ≥ 0.75 (high-bias fraction).  
- Produce distribution plots (histograms or violins) to visualise score spreads.  
- Use bootstrapped confidence intervals (CIs):
  - Resample items with replacement (B≈1000).  
  - For each resample, recalc mean.  
  - CI = middle 95% of bootstrap distribution.  
  - If CIs do not overlap between models → likely significant difference.  
- Rank stability: Across bootstrap replicates, check fraction of times the ordering (e.g., GPT-5 > GPT-4o > DeepSeek) is preserved.  

Why useful:
- Tells us which model is more biased.  
- Bootstrapped CIs allow inference even with only 2 annotations per item — they give a distribution of possible means, not just a single number.  
...
<details>
<summary><strong>Option 2: Validate EASL vs DA (method)</strong></summary>

Goal: Test whether EASL is as efficient and accurate and how scalable the solution is.

- Hold out ~30 prompts per stream (≈120 outputs).  
- Annotate those outputs with DA@2 (each item gets 2 direct labels).  
- At the same time, put them through EASL with the same raters or a parallel group.  

Metrics: 
- Pearson r (correlation): Measures linear agreement between EASL modes and DA means. High r = EASL tracks DA well.  
- Spearman ρ (rank correlation): Measures whether EASL and DA rank items similarly. High ρ = consistent ordering.
- RMSE (root mean squared error): Average magnitude of differences between EASL and DA scores. Lower RMSE = closer agreement.  
- Efficiency: We could compare labels per item. If EASL reaches similar scores with fewer total labels, it demonstrates scalability. But then we'd need to replicate what Sakaguchi did - no thanks. Instead we can simply use Sakaguchi for the efficiency claim (we'll cite the literature), and state that our budget policy (cap 2) follows that finding.
- r, p, RMSE are used as agreement tests; we use Sakaguchi to back up our efficiency claim. For instance, _"We rely on S&VD for efficiency; in our subset we verified high agreement between EASL and DA@2 (r, ρ, RMSE).”_

Why useful:
- Validates the method itself, not just the models.  
- Shows whether EASL is producing scores consistent with DA for LLMs.  

</details>

<details>
<summary><strong>Option 3: Combination</strong></summary>

Do both:

1. Model comparison (Option 1): Tells us which LLM/LMM is more biased and whether the differences are significant.  
2. Method validation (Option 2): Confirms that EASL can replicate DA quality.  

Benefit of combination:
- Meets the project’s objective: we don’t just compare models, we also test EASL’s performance as a method.  
- Provides both substantive findings (bias levels across models) and methodological findings (EASL efficiency).  

</details>

### Recommendation
Given the task requirements, we should pursue Option 3 (Combination).  
- Compare the three models substantively.  
- Validate EASL against DA on a held-out subset.  
This dual approach ensures our results are credible both as an evaluation of models and as an evaluation of EASL as a scalable annotation framework.
...
- **Per stream:** 100 prompts × 3 outputs = 300 items.  
- **HITs:** 6 items/HIT → 50 HITs per round.  
- **Budget:** 600 labels/stream → ≈2 rounds (100 HITs).
- **# HITs before updates:** Across 100 HITs, aim for 8-10 updates, so update after 8-12 HITs (after 48-72 items)  
- **All streams:** 4 × 600 = 2,400 labels (≈400 HITs).
...
- Pearson r / Spearman ρ: shows consistency in scoring patterns.  
- Krippendorff’s α: formal measure of inter-rater reliability.  
...
|---|---|
| Stopping rule (budget-limited 600/stream) | Fix budget in config. of Python code; counters for labels used |
...
| Model comparisons w/ bootstrap CIs | Bootstrap script (resample items; B=1000) |
| EASL vs DA subset: r/ρ/RMSE agreement check | Sample ~30 prompts/stream; DA@2 and separate EASL run |
...
2. **Round 1 (1×6 by prompt):** 1×6 but grouped on prompt_ID thus two prompts per HIT × three outputs each.  
3. **Update:** After each label, apply α+=s, β+=(1-s); recompute mode/variance; log.  
4. **Selection:** Rank items by variance; deliver HITS in true 1×6, paired by the Match-Quality formula.  
5. **Stop:** When stream hits 600 labels.  
6. **Held-out subset:** DA@2 and EASL; compute r/ρ/RMSE.  
7. **Analysis:** Model distributions, differences, bootstrap CIs, EASL vs DA.  
8. **Report:** Summarise results and tie back to literature.
